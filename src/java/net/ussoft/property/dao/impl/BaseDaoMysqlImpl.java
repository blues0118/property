...
import net.ussoft.property.base.BaseDao;
import net.ussoft.property.base.DaoException;
import net.ussoft.property.model.PageBean;
import net.ussoft.property.util.BeanUtilsBean;
...
public abstract class BaseDaoMysqlImpl<T,ID> extends JdbcDaoSupport implements BaseDao<T,ID>{
	...
	public List<T> search(String sql,List<Object> values){
		...
		List<T> list = this.getJdbcTemplate().query(sql, values.toArray(),new BeanPropertyRowMapper<T>(this.persistentClass));
	...
	public int add(String sql,List<Object> values){
		...
		int a = this.getJdbcTemplate().update(sql, values.toArray());
		return a;
//		return this.getJdbcTemplate().update(sql, values.toArray());
	...
	public  ID addReturnId(final String sql,final List<Object> values){
		...
		return (ID)(String)keyHolder.getKey().toString();
	}
	@SuppressWarnings("unchecked")
...
//		sb.append("select count(*) from ").append(this.tableName).append(" ").append(sql);
	...
	protected int updateById(Map<String, Object> map){
		...
		if(log.isDebugEnabled()){
			log.debug("update : "+sb.toString()+" values:"+values);
		}
	...
	public <E> List<E> search(String sql, List<Object> values, Class<E> e) {
		...
		if(values==null)values=new ArrayList<Object>();
		logger.info("sql : "+sql+" values:"+ values);
		return this.getJdbcTemplate().query(sql, values.toArray(),new BeanPropertyRowMapper<E>(e));
	}
	@Override
