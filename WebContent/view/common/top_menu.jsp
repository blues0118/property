	           	...
	           	<ul>
	                ...
	                <c:forEach items="${functions}" varStatus="i" var="item">
						<c:if test="${item.funparent == '0' }">
							<li class="${item.funencode==focus_first?'onthis':''}"  ><a href="${pageContext.request.contextPath}/${item.funpath }">${item.funcode }</a></li>
						</c:if>
			        </c:forEach>
			        <li><a href="#">帮助</a></li>
	            </ul>
			...
			<div id="top_nav">
		        ...
		        <ul>
		        	<c:forEach items="${functions}" varStatus="i" var="item">
						...
						<li class="${item.funencode==focus_second?'nowcolor':''}"  ><a class="${item.funencode==focus_second?'nowcolor':''}" href="${pageContext.request.contextPath}/${item.funpath }">${item.funcode }</a></li>
						</c:if>
			        </c:forEach>
		        </ul>
